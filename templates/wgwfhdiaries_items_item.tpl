...
<div class='panel-foot'>
	<div class='col-sm-12 right'>
		<{if $showItem|default:''}>
			<a class='btn btn-success right' href='items.php?op=list&amp;#itemId_<{$item.item_id}>' title='<{$smarty.const._MA_WGWFHDIARIES_ITEMS_LIST}>'><{$smarty.const._MA_WGWFHDIARIES_ITEMS_LIST}></a>
		<{else}>
			<a class='btn btn-success right' href='items.php?op=show&amp;item_id=<{$item.item_id}>' title='<{$smarty.const._MA_WGWFHDIARIES_DETAILS}>'><{$smarty.const._MA_WGWFHDIARIES_DETAILS}></a>
		<{/if}>
		<{if $permEdit|default:''}>
